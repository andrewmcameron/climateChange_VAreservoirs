...
mblm_fits <- pre70 %>%
  ...
  summarise(intercept = coef(model)[1], slope = coef(model)[2], "p val" = mblm::summary.mblm(model)$coefficients[2,4], Month = first(Month)) %>%
...
annotation_data <- data.frame(
  ...
  x = c(2010,2010,2010, 1982, 1982, 1982), 
  y = c(-4, -3.75, -3, -3, -3, -3),  
  label = my_labels)
...
ggplot(data = plottingData) +
  ...
  facet_wrap(~factor(Month), scales = "free_y") +
  geom_text(data = annotation_data, aes(x = x, y = y, label = label), inherit.aes = FALSE, check_overlap = TRUE) +
