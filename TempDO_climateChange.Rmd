---
title: "Temperature in VA Reservoirs"
...
monthlyStation_stats %>%
  select(-key) -> deliverable

openxlsx::write.xlsx(deliverable, "TMax_monthlyByStation_mblmModelStatistics.xlsx")
```
...
length(which(deliverable$model_pval < .06))

df %>%
  filter(model_pval < 0.05) %>%
  filter(model_slope > 0 ) -> signif.pVal_pos.Slope

summary(signif.pVal_pos.Slope$model_slope)
