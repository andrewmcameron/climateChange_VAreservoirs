...
# exclude station years with fewer than 3 obs
subset.df <- subset.df %>%
  filter(!stationYear %in% yrs_insufObs$stationYear)


# Identify station years lacking May or August observations
MayAugust_present <- subset.df%>%
  group_by(FDT_STA_ID, year(Date)) %>%
  summarize(May_obs = any(lubridate::month(Date) == 5),
            Aug_obs = any(lubridate::month(Date) == 8)) %>%
  filter(May_obs == TRUE & Aug_obs == TRUE) %>%
  mutate(staYear = paste0(FDT_STA_ID, "-", `year(Date)`)) # Create station-year field to facilitate filtering

staYrs_MayAug <- MayAugust_present$staYear

data_MayAug <- subset.df %>%
  filter(stationYear %in% staYrs_MayAug)

```


# May to August Modelling
...
MayAug_present_tmin <- TMin_data %>%
  group_by(FDT_STA_ID, year(Date)) %>%
  summarize(Apr_obs = any(lubridate::month(Date) == 5),
            Aug_obs = any(lubridate::month(Date) == 8)) %>%
...
staYrs_TMin <- MayAug_present_tmin$stationYear
